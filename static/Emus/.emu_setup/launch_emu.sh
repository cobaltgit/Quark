...
run_retroarch() {
    ...
    if [ -n "$NET_PARAM" ]; then
        if [ "$(awk -F ':' '/wifi/ {print $2}' "/mnt/UDISK/system.json" | sed 's/^[[:space:]]*//; s/[",]//g')" -eq 0 ]; then
            ...
            case "$CORE" in
                a5200|ardens|atari800|bluemsx|dosbox_pure|easyrpg|ecwolf|fake08|fbalpha2012_cps3|freechaf|freeintv|fuse|gametank|gw|hatari|neocd|pokemini|potator|prboom|prosystem|px68k|tic80|tyrquake|uae4arm|vice_x64|vice_xvic)
                    NET_PARAM= ;;
            esac
        fi
    fi
