...
proc main() =
        ...
        if i + 1 <= paramCount():
          ...
          if durationStr.len > 0:
            try:
              ...
              stderr.writeLine("display: invalid duration value: '" & durationStr & "'")
              quit(1)
          inc i
        else:
  ...
  if not hasText or text.len == 0:
    stderr.writeLine("display: no text provided (use -t \"your text\" or provide text as argument)")
    showUsage(getAppFilename())
    quit(1)
  
  let fbFd = posix.open("/dev/fb0", O_RDWR)
  if fbFd < 0:
    stderr.writeLine "display: failed to open /dev/fb0"
    quit(1)
  defer: discard close(fbFd)
  
  let fbMap = mmap(nil, FbSize, PROT_READ or PROT_WRITE, MAP_SHARED, fbFd, 0)
  if fbMap == MAP_FAILED:
    stderr.writeLine "display: failed to mmap framebuffer"
    quit(1)
  ...
  if not fileExists(backgroundPath):
    stderr.writeLine("display: background file not found: " & backgroundPath)
    quit(1)
  ...
  try:
    ...
    if png.width != ScreenWidth or png.height != ScreenHeight:
      stderr.writeLine("display: background is " & $png.width & "x" & $png.height & 
                       ", expected 320x240")
    
    var srcIdx = 0
    for ly in 0..<min(png.height, SCREEN_HEIGHT):
      for lx in 0..<min(png.width, SCREEN_WIDTH):
        let r = png.data[srcIdx]
        let g = png.data[srcIdx + 1]
        let b = png.data[srcIdx + 2]
        srcIdx += 4
        
        let fbIdx = RowOffsets[RotationXMap[lx]] + ly
        fb[fbIdx] = toRGB565(r.uint8, g.uint8, b.uint8)
    ...
    stderr.writeLine("display: failed to load background: " & e.msg)
    quit(1)
  
  if not fileExists(fontPath):
    stderr.writeLine("display: font file not found: " & fontPath)
    quit(1)
  
  let fontData = loadFont(fontPath)
  var fontInfo: stbtt_fontinfo
  if stbtt_InitFont(addr fontInfo, cast[ptr UncheckedArray[byte]](unsafeAddr fontData[0]), 0) == 0:
    stderr.writeLine("display: failed to load font: " & fontPath)
    quit(1)
  ...
  let totalHeight = lines.len * lineHeight
  var startY = (ScreenHeight - totalHeight) div 2
  
  let white = toRGB565(255, 255, 255)
  
  for line in lines:
    renderTextLine(fb, addr fontInfo, line, startY, FontSize, white)
    startY += lineHeight
